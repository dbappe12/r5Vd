(function ($) {
  ...
  $(window).on("scroll", function () {
    if ($(".stricked-menu").length) {
      var headerScrollPos = 130;
      var stricky = $(".stricked-menu");
      if ($(window).scrollTop() > headerScrollPos && !isZoomModalOpen()) {
        stricky.addClass("stricky-fixed");
        stricky.css("z-index", "9999"); // Default sticky z-index
      } else {
        stricky.removeClass("stricky-fixed");
        stricky.css("z-index", ""); // Reset z-index
      }
    }
    // ... (Other scroll-related code unchanged: scroll-to-top, OnePageMenuScroll) ...
  });

  // Listen for zoom modal open/close
  $(document).on("click", ".zoom-image", function (e) {
    e.preventDefault();
    var modal = $("#zoomModal");
    var modalImg = $("#zoomImage");
    modal.show();
    modalImg.attr("src", $(this).data("image"));
    // Lower navbar z-index
    $(".stricked-menu, .stricked-menu.stricky-fixed").css("z-index", "1000");
  });

  $(document).on("click", ".zoom-modal-close, #zoomModal", function (e) {
    if (e.target === this || $(e.target).hasClass("zoom-modal-close")) {
      $("#zoomModal").hide();
      $("body").css("overflow", "auto");
      // Restore navbar z-index
      $(".stricked-menu, .stricked-menu.stricky-fixed").css("z-index", "");
    }
  });
